...
window_pixel_scale :: proc(window: ^SDL.Window) -> [2]f32 {
	...
	return {f32(pixel_width) / f32(window_width), f32(pixel_height) / f32(window_height)}
}
...
main :: proc() {
	...
	for running {
		...
		for SDL.PollEvent(&event) {
			...
			case .TEXT_INPUT:
				text_input, _ := strings.clone_from_cstring(
					event.text.text,
					context.temp_allocator,
				)
				if len(text_input) > 0 {
		...
		if mui.begin_window(ui_ctx, "Debug Info", mui.Rect{100, 100, 400, 400}) {
			...
			mui.layout_row(
				ui_ctx,
				[]i32{win_size.x - ui_ctx.style.padding / 2},
				mui.get_layout(ui_ctx).size.y,
			)
		...
		if drag_and_drop {
			screen_w, screen_h :=
				ctx.base.Window.last_extent.width, ctx.base.Window.last_extent.height
			if mui.begin_window(
				ui_ctx,
				"Drag and Drop",
				mui.Rect{cast(i32)20, cast(i32)20, cast(i32)screen_w - 40, cast(i32)screen_h - 40},
			) {
				win_size := mui.get_layout(ui_ctx).size
				pad := mui.get_layout(ui_ctx).indent
				mui.layout_row(
					ui_ctx,
					[]i32{win_size.x - ui_ctx.style.padding / 2},
					win_size.y - ui_ctx.style.padding / 2,
				)
				mui.end_window(ui_ctx)
			}
		}
		...
		if len(app_context.csv_files) > 0 {
			if mui.begin_window(ui_ctx, "Files", mui.Rect{40, 40, 400, 240}) {
				...
				for filename, file_index in app_context.csv_files {
					stripped_file_name, err := strings.split_after(
						filename,
						"/",
						context.temp_allocator,
					)
					...
					if .SUBMIT in mui.button(ui_ctx, button_label) &&
					   button_label == "Not Processed" {
		...
		if len(graph_data_global) > 0 {
			...
			upload_data := make(
				[]app.GraphGpuData,
				len(graph_data_global) + 2,
				context.temp_allocator,
			)
			x_off: f32 = 20.0
			col_width :=
				(cast(f32)frame.extent.width - 2 * x_off) / cast(f32)len(graph_data_global)
			for data, idx in graph_data_global {
				upload_data[idx] = app.ui_graph_charts_formalize_data(
					&graph_renderer,
					x_off,
					cast(f32)frame.extent.height - 10.0,
					col_width,
					data,
				)
				ndc_mouse_pos: [2]f32 = {
					(cast(f32)ui_ctx.mouse_pos.x / cast(f32)frame.extent.width) * 2.0 - 1.0,
					(cast(f32)ui_ctx.mouse_pos.y / cast(f32)frame.extent.height) * 2.0 - 1.0,
				}
				if ndc_mouse_pos.x >= upload_data[idx].top_left.x &&
				   ndc_mouse_pos.x <= upload_data[idx].bottom_right.x {
					if ndc_mouse_pos.y >= upload_data[idx].top_left.y &&
					   ndc_mouse_pos.y <= upload_data[idx].bottom_right.y {
			...
			//
			upload_data[len(graph_data_global)] = app.ui_graph_charts_formalize_data(
				&graph_renderer,
				20,
				cast(f32)frame.extent.height - 10.0,
				5.0,
				cast(f32)frame.extent.height / 2.0,
				{0.1, 0.1, 0.55, 1.0},
				{0.1, 0.1, 0.35, 1.0},
			)
			upload_data[len(graph_data_global) + 1] = app.ui_graph_charts_formalize_data(
				&graph_renderer,
				20,
				cast(f32)frame.extent.height - 10.0,
				cast(f32)frame.extent.width - 40.0,
				5.0,
				{0.1, 0.1, 0.55, 1.0},
				{0.1, 0.1, 0.35, 1.0},
			)
		...
		gpu.gpu_begin_swapchain_rendering(
			ctx,
			frame,
			{clear = true, color = {0.02, 0.025, 0.04, 1.0}},
		)
